...
cp /opt/DMRGateway/DMRGateway ${BIN} > /dev/null 2<&1
#cp /opt/DMRGateway/DMRGateway.ini ${SETC}dmrgateway > /dev/null 2<&1
cp /opt/MMDVMCal/MMDVMCal ${BIN} > /dev/null 2<&1
cp /opt/MMDVMHost/MMDVMHost ${BIN} > /dev/null 2<&1
#cp /opt/MMDVMHost/MMDVM.ini ${SETC}mmdvmhost > /dev/null 2<&1
cp /opt/MMDVMHost/RemoteCommand ${BIN} > /dev/null 2<&1
...
curl --fail -o ${SBIN}watchdog.service -s ${MURL}sbin/watchdog.service > /dev/null 2<&1
curl --fail -o ${SBIN}watchdog -s ${MURL}sbin/watchdog > /dev/null 2<&1
curl --fail -o ${SBIN}findmodem -s ${MURL}sbin/findmodem > /dev/null 2<&1
curl --fail -o ${SBIN}mmdvmcal -s ${MURL}sbin/mmdvmcal > /dev/null 2<&1
...
curl --fail -o ${LIB}watchdog.service -s ${MURL}system/watchdog.service > /dev/null 2<&1
curl --fail -o ${LIB}watchdog.timer -s ${MURL}system/watchdog.timer > /dev/null 2<&1
...
systemctl enable watchdog.service > /dev/null 2<&1
systemctl enable watchdog.timer > /dev/null 2<&1
